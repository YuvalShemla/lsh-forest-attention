#!/usr/bin/env python3
"""
SimHash vs Cross-Polytope LSH Parameter Sweep

Compares SimHash SNIS and Cross-Polytope SNIS across a grid of (K, L, min_hits)
configurations, with budget-controlled baselines (TopK, Uniform, Oracle).
...
def _snis_estimator(logits, values, inclusion_probs):
    """SNIS estimator (local version matching HEAD_DIM)."""
    if len(logits) == 0:
...
# ============================================================================
# BASELINE METHODS (optimized local versions for percentage-based budgets)
# ============================================================================
...
def do_snis(query, keys, values, logits, retrieved_idx, collision_prob_fn,
    ...
    inc = np.clip(_inclusion_prob(p_table, L_use, mh), 1e-12, 1.0)
    return _snis_estimator(r_logits, r_values, inc), n
...
def analyze_example(example, layer_name, sh_index, cp_index, rng):
    """Analyze one example: compute baselines and all LSH configs per query."""
...
def plot_baselines(agg_first, agg_last, output_dir):
    ...
    for ax, agg, title in [
        ...
        for method, label, color, marker, ls in [
            ...
            means = np.array([agg['baselines'][method][str(k)]['mean']
                              for k in K_PERCENTAGES])
            stds = np.array([agg['baselines'][method][str(k)]['std']
                             for k in K_PERCENTAGES])
    ...
    fig.suptitle('Baseline Methods: TopK vs Uniform vs Oracle',
                 fontweight='bold', fontsize=14, y=1.01)
    plt.tight_layout()
    fig.savefig(output_dir / 'baselines.png', dpi=200,
                bbox_inches='tight', facecolor='white')
    plt.close()


def plot_scatter_by_minhits(agg, layer_label, mh_val, output_dir):
    ...
    for k_cp in CP_K_VALUES:
        ...
        for L in L_vals:
            ...
            if d['n'] > 0 and d['mean_budget'] > 0:
                ...
                labs.append(f"L={L}")
        if bx:
            ax.scatter(bx, by, color=cp_colors[k_cp], marker='*', s=90,
                       alpha=0.85, label=f'CrossPoly k={k_cp}', zorder=5)
            for xi, yi, lab in zip(bx, by, labs):
                ax.annotate(lab, (xi, yi), fontsize=5.5, alpha=0.5,
                            xytext=(3, 3), textcoords='offset points')
    ...
    ax.set_ylim(bottom=0)
    plt.tight_layout()
    fig.savefig(output_dir / f'scatter_{layer_label.lower().replace(" ", "_")}_h{mh_val}.png',
                dpi=200, bbox_inches='tight', facecolor='white')
    plt.close()


def plot_minhits_comparison(agg, layer_label, output_dir):
    ...
    # Right: CrossPoly k=1
    ax = axes[1]
    ax.set_title(f'CrossPoly k=1: Effect of min_hits ({layer_label})',
                 fontweight='bold')
    for mh, marker, alpha in [(1, 'o', 0.5), (2, 's', 0.8), (3, '^', 1.0)]:
        bx, by = [], []
    ...
    plt.tight_layout()
    fig.savefig(output_dir / f'minhits_comparison_{layer_label.lower().replace(" ", "_")}.png',
                dpi=200, bbox_inches='tight', facecolor='white')
    plt.close()
...
def main():
    ...
    print(f"Shared index: SimHash [{MAX_SH_L}T, {MAX_SH_K}K], "
          f"CP [{MAX_CP_L}T, {MAX_CP_K}k]")
    ...
    # Build shared LSH structures
    print("Building shared LSH structures...")
    sh_idx = SimHashIndex(MAX_SH_L, MAX_SH_K, HEAD_DIM,
                          center_keys=True, seed=SEED)
    cp_idx = CrossPolytopeIndex(MAX_CP_L, MAX_CP_K, HEAD_DIM,
    ...
    plot_baselines(aggregated['first_layer'], aggregated['last_layer'],
                   OUTPUT_DIR)

    for layer in LAYERS:
        ll = 'First Layer' if 'first' in layer else 'Last Layer'
        for mh in MIN_HITS_VALUES:
            plot_scatter_by_minhits(aggregated[layer], ll, mh, OUTPUT_DIR)

        plot_minhits_comparison(aggregated[layer], ll, OUTPUT_DIR)
    ...
    for m in ['topk', 'uniform', 'oracle']:
        ...
        print(f"{m.capitalize() + ' @10%':<35} {'~600':>8} "
              f"{d['mean']:>8.4f} {d['median']:>8.4f} {'0.0%':>7}")
