...
Takes aggregated data and generates publication-quality plots,
then builds HTML dashboard.

Can be run standalone to regenerate dashboard from saved aggregated data.
"""
...
def create_all_plots(data_first, data_last):
    ...
    for ax, data, title in [(axes[0], data_first, 'First Layer (Layer 0)'),
        ...
        x = data['conc_x']
        ax.plot(x, data['conc_mean'], label='Mean', color='black', linewidth=3.5, alpha=0.9, zorder=5)
        ax.plot(x, data['conc_p50'], label='Median (p50)', color='#3b82f6', linewidth=2.5, linestyle='--', alpha=0.85, zorder=4)
        ax.plot(x, data['conc_p10'], label='p10', color='#ef4444', linewidth=2, linestyle=':', alpha=0.75, zorder=3)
        ax.plot(x, data['conc_p90'], label='p90', color='#22c55e', linewidth=2, linestyle=':', alpha=0.75, zorder=3)
        ax.plot(x, data['conc_p99'], label='p99', color='#a855f7', linewidth=1.8, linestyle='-.', alpha=0.7, zorder=2)
        ax.fill_between(x, data['conc_p10'], data['conc_p90'], alpha=0.15, color='gray', zorder=1)
        ax.plot([0, 100], [0, 100], 'k--', alpha=0.35, linewidth=1.5, label='Uniform', zorder=0)
        
        ax.set_xlabel('% of Keys (sorted by attention weight)', fontweight='bold', fontsize=12)
        ax.set_ylabel('% of Attention Mass Captured', fontweight='bold', fontsize=12)
        ax.set_title(title, fontweight='bold', pad=12, fontsize=13)
        ax.set_xlim([0, 100])
        ax.set_ylim([0, 105])
        ax.grid(True, alpha=0.25, linestyle='-', linewidth=0.5)
        ax.legend(loc='lower right', framealpha=0.95, fontsize=10, edgecolor='black')
        ...
        ax.annotate(f'{mean_at_10:.1f}%\\nat 10% keys',
                    ...
                    arrowprops=dict(arrowstyle='->', lw=1.5, color='black'), fontsize=10, fontweight='bold')
    ...
    for row, (data, layer_title) in enumerate([(data_first, 'First Layer'), (data_last, 'Last Layer')]):
        for col, (norm_key, label, color) in enumerate([
            ...
            ax = axes[row, col]
            norms = data[norm_key]
            ax.hist(norms, bins=100, color=color, alpha=0.75, edgecolor='black', linewidth=0.6)
            mean_val = np.mean(norms)
            median_val = np.median(norms)
            ax.axvline(mean_val, color='red', linestyle='--', linewidth=2.5, label=f'Mean: {mean_val:.2f}', alpha=0.9)
            ax.axvline(median_val, color='blue', linestyle=':', linewidth=2.5, label=f'Median: {median_val:.2f}', alpha=0.9)
            ax.set_xlabel('L2 Norm', fontweight='bold')
            ax.set_ylabel('Frequency', fontweight='bold')
            ax.set_title(f'{layer_title} — {label}', fontweight='bold', fontsize=11)
    ...
    fig.suptitle('Vector Normalization: Are Q, K, V Normalized?', fontsize=15, fontweight='bold', y=0.995)
    plt.tight_layout()
    ...
    for row, (data, layer_title) in enumerate([(data_first, 'First Layer'), (data_last, 'Last Layer')]):
        ...
        ax.hist(dists, bins=100, color='#6366f1', alpha=0.75, edgecolor='black', linewidth=0.5)
        ...
        ax.set_xlabel('L2 Distance ||k - q||₂', fontweight='bold', fontsize=11)
        ax.set_ylabel('Frequency', fontweight='bold')
        ...
        ax.hist(cos, bins=120, color='#22d3ee', alpha=0.75, edgecolor='black', linewidth=0.5)
        ...
        ax.set_xlabel('Cosine Similarity (k · q)/(||k|| ||q||)', fontweight='bold', fontsize=11)
        ax.set_ylabel('Frequency', fontweight='bold')
        ax.set_title(f'{layer_title} — Cosine Similarity', fontweight='bold')
        ax.legend(framealpha=0.95, fontsize=9, edgecolor='black')
        ax.grid(True, alpha=0.2)
        
        # Stats box for cosine
        ax.text(0.02, 0.97, f'Std: {np.std(cos):.3f}\\np10: {np.percentile(cos, 10):.3f}\\np90: {np.percentile(cos, 90):.3f}', 
                transform=ax.transAxes, ha='left', va='top', fontsize=9,
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.9, edgecolor='gray'))
    
    fig.suptitle('Key-Query Distance Distributions', fontsize=15, fontweight='bold', y=0.995)
    plt.tight_layout()
    ...
    for row, (data, layer_title) in enumerate([(data_first, 'First Layer'), (data_last, 'Last Layer')]):
        ...
        if len(data['kv_corr_l2']) > 0:
            ...
            ax.hist(corrs, bins=60, color='#f59e0b', alpha=0.75, edgecolor='black', linewidth=0.6)
            ax.axvline(np.mean(corrs), color='red', linestyle='--', linewidth=2.5,
                       label=f"Mean: {np.mean(corrs):.3f}")
            ax.axvline(np.median(corrs), color='blue', linestyle=':', linewidth=2.5,
                       label=f"Median: {np.median(corrs):.3f}")
            ax.set_xlabel('Pearson r (L2 Key-Dist ↔ L2 Value-Dist)', fontweight='bold', fontsize=10)
            ax.set_ylabel('Frequency', fontweight='bold')
        ...
        if len(data['kv_corr_cos']) > 0:
            ...
            ax.hist(corrs, bins=60, color='#22d3ee', alpha=0.75, edgecolor='black', linewidth=0.6)
            ax.axvline(np.mean(corrs), color='red', linestyle='--', linewidth=2.5,
                       label=f"Mean: {np.mean(corrs):.3f}")
            ax.axvline(np.median(corrs), color='blue', linestyle=':', linewidth=2.5,
                       label=f"Median: {np.median(corrs):.3f}")
            ax.set_xlabel('Pearson r (Cos Key-Dist ↔ Cos Value-Dist)', fontweight='bold', fontsize=10)
            ax.set_ylabel('Frequency', fontweight='bold')
            ax.set_title(f'{layer_title} — Cosine Correlation', fontweight='bold')
            ax.legend(framealpha=0.95, fontsize=9, edgecolor='black')
            ax.grid(True, alpha=0.2)
    
    fig.suptitle('Key-Value Pairwise Distance Correlation (Top-100 Keys)', fontsize=15, fontweight='bold', y=0.995)
    plt.tight_layout()
    ...
    for ax, data, layer_title in [(axes[0], data_first, 'First Layer'), (axes[1], data_last, 'Last Layer')]:
        ...
        bars = ax.bar(positions, means, color=colors_topk, alpha=0.8, edgecolor='black', linewidth=1.8, width=0.7)
        ax.errorbar(positions, means, yerr=stds, fmt='none', color='black', linewidth=2.5, capsize=7, capthick=2, alpha=0.8)
        ...
        ax.set_ylabel('Attention Mass Captured (%)', fontweight='bold', fontsize=12)
        ax.set_title(layer_title, fontweight='bold', pad=12, fontsize=13)
        ax.set_ylim([0, max(means) + max(stds) + 15])
        ax.grid(True, alpha=0.25, axis='y', linestyle='--')
    
    fig.suptitle('Top-K Mass: How Much Attention is Concentrated?', fontsize=15, fontweight='bold', y=1.0)
    plt.tight_layout()
    plots['topk_mass'] = fig_to_base64(fig, dpi=200)
    
    # ============================================================
    # 6. PER-EXAMPLE VARIANCE (showing cross-example variability)
    # ============================================================
    ...
    for idx, (metric_key, metric_label, color) in enumerate(metrics):
        ...
        # Combine both layers for comparison
        vals_first = data_first['per_example_stats'].get(metric_key, [])
        vals_last = data_last['per_example_stats'].get(metric_key, [])
        
        if len(vals_first) > 0:
            ax.hist(vals_first, bins=25, color='#3b82f6', alpha=0.6, edgecolor='black', linewidth=0.6, label='First Layer')
        if len(vals_last) > 0:
            ax.hist(vals_last, bins=25, color='#ef4444', alpha=0.6, edgecolor='black', linewidth=0.6, label='Last Layer')
    ...
    fig.suptitle('Comprehensive Statistics Summary', fontsize=15, fontweight='bold', y=0.97)
    plt.tight_layout()
...
<div class="container">
  ...
  <div class="content">
    ...
    <div class="section">
      ...
      <div class="description">
        ...
        <code>||v|| = sqrt(sum(v_i²))</code>. Reveals whether vectors are normalized to a constant scale.<br>
    ...
    <div class="section">
      ...
      <div class="description">
        ...
        <strong>Left:</strong> L2 (Euclidean) distance <code>||k - q||₂</code>. 
    ...
    <div class="section">
      ...
      <div class="description">
        ...
        (K ∈ {{10, 50, 100, 200}}), averaged across all queries. Error bars show standard deviation.<br>
  ...
  <footer>
    <p><strong>Professional Attention Analysis Dashboard</strong> · Llama-3-8B · LongBench v2</p>
