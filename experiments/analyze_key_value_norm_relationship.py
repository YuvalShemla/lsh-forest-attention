...
2. How does ||K|| / ||V|| ratio affect approximation errors?
3. Should we normalize for fair comparison?
4. Does high ||key|| imply high ||value|| (and vice versa)?

Output: Plots and statistics showing the relationship.
"""
...
def create_plots(data_first, data_last):
    ...
    for col, (data, layer_title) in enumerate([(data_first, 'First Layer (Layer 0)'), 
        ...
        ax.set_xlabel('||Key||₂', fontweight='bold', fontsize=11)
    ...
    for col, (data, layer_title) in enumerate([(data_first, 'First Layer'), (data_last, 'Last Layer')]):
        ...
        ax.set_xlabel('||Key||₂ (for top-10% keys by logit)', fontweight='bold', fontsize=11)
        ax.set_ylabel('||Value||₂ (corresponding values)', fontweight='bold', fontsize=11)
